...
const Login = () => {
  ...
  return (
    ...
    <Header />
    <form className="flex flex-col items-center h-2/5 h-auto  bg-slate-500 bg-opacity-75  shadow-2xl w-8/12 rounded-3xl p-6" onSubmit={handleLogin}> 
        ...
        <p className='text-[1.3rem] text-white'>Dont have an account? <Link to={'/signup'}>Signup</Link></p>
        <button type='submit' className='text-3xl pr-12 pl-12 pb-4 pt-4 text-center rounded-2xl mb-5 text-white hover:bg-blue-600 hover:scale-90 bg-blue-400'>Login</button>
      </form>
  </div>
  )
}
